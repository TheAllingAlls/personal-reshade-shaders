...
=============================================================================*/
 ...
 #define SMOOTHNORMALS 			0   //[0 or 1]      0: off | 1: enables some filtering of the normals derived from depth buffer to hide 3d model blockyness
#endif
...
#if IMAGEBASEDLIGHTING != 0 //need to toggle textures and samplers off, thank you DX9
texture ProbeTex      					        { Width = 256;   Height = 256;  Format = RGBA16F;};
sampler sProbeTex	    					    { Texture = ProbeTex;	};
texture ProbeTexPrev      					    { Width = 256;   Height = 256;  Format = RGBA16F;};
...
if(RT_DO_RENDER) 
{
        ...
        parameters.jitter = Random::goldenweyl3((qUINT::FRAME_COUNT % 3000u) * parameters.nrays + r, parameters.jitter);
        ...
        float cos_view = dot(-parameters.eyedir, ray.dir);
        ray.steplen = (ray.maxlen / parameters.nsteps);
        ray.steplen *= rsqrt(saturate(1.0 - cos_view * cos_view) + 0.05);
        
        //yeah that's ugly. No, I will keep it. TODO: change RT logic to not use lambda anymore
        float3 srcpos = parameters.pos;
        float3 destpos = srcpos + ray.steplen * ray.dir;
        float2 delta = Projection::proj_to_uv(destpos) - Projection::proj_to_uv(srcpos);
        float fact = length(delta) + 0.0000001;
        ray.steplen *= max(1, length(qUINT::PIXEL_SIZE) / fact * 2.0); //set minimum step length 2 pixels
...
{
    o = Denoise::filter(i, sRTGITempTex0, 0, 0 + SKIP_FILTER);
}
void PS_Filter1(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sRTGITempTex1, 1, RT_DO_RENDER + SKIP_FILTER);
}
void PS_Filter2(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sRTGITempTex0, 2, RT_DO_RENDER + SKIP_FILTER);
}
void PS_Filter3(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sRTGITempTex1, 3, RT_DO_RENDER + SKIP_FILTER);
}
...
void PS_Blending(in VSOUT i, out float4 o : SV_Target0)
{
    ...
    float3 skycol = tex2Dfetch(sSkyCol, 0).rgb * SKY_COLOR_TINT;
    skycol = lerp(dot(skycol, 0.333), skycol, SKY_COLOR_SAT * 0.2);
 #endif
    skycol *= fade;
...
void PS_Probe(in VSOUT i, out float4 o : SV_Target0)
{
    ...
    uint search_size_spatial    = 16;
    uint search_size_temporal   = 64;
    uint frame_num              = qUINT::FRAME_COUNT;

    float2 grid_shift = Random::goldenweyl2(frame_num % search_size_temporal);  

    grid_shift = lerp(0.05, 0.95, grid_shift);

    float grid_increment = 1.0 / float(search_size_spatial); //blocksize in % of screen

    float2 grid_start = grid_shift * grid_increment;
    float2 grid_pos = grid_start;

    float totalw = 0;

    for(int x = 0; x < search_size_spatial; x++)
    {
        for(int y = 0; y < search_size_spatial; y++)
        {
            float4 tapg = tex2Dlod(sGBufferTex, float4(grid_pos, 0, 0));
            float4 tapc = tex2Dlod(sColorTex, float4(grid_pos, 0, 0));
            float similarity = pow(saturate(dot(tapg.xyz, -n)), 4); //reduces intensity
            similarity *= tapc.a; //mask sky
            //similarity *= Projection::z_to_depth(tapg.w) * tempF1.x;
            //similarity = saturate(dot(tapg.xyz, -n) * tempF1.x - tempF1.x + 1.0); //-> use for rotation reconstruction!
            unpack_hdr(tapc.rgb);  
            //o += float4(tapc.rgb, 1) * similarity;

            float w = lerp(0.01, 1.0, saturate(Projection::z_to_depth(tapg.w)));

            o += float4(tapc.rgb * similarity, similarity) * w;
            totalw += w;
            grid_pos.y += grid_increment;          
        }
        grid_pos.y = grid_start.y;
        grid_pos.x += grid_increment;
    }

    o /= totalw;
    o *= 3.0;
    //o /= search_size_spatial * search_size_spatial; 
    //o.rgb /= 0.73882 * 2; //precomputed area under sqrt(1-x*x)^10 from -1 to 1, cannot be computed on the fly as the integral of sqrt(1-x*x)^k involves the Gamma function, for which no closed form exists
    pack_hdr(o.rgb);
    o = lerp(tex2D(sProbeTexPrev, i.uv), o, 0.02);
}
